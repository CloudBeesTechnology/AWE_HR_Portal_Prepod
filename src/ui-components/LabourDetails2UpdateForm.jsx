...
import { getLabourDetails2 } from "../graphql/queries";
import { updateLabourDetails2 } from "../graphql/mutations";
...
export default function LabourDetails2UpdateForm(props) {
  const {
    id: idProp,
    labourDetails2: labourDetails2ModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = labourDetails2Record
      ? { ...initialValues, ...labourDetails2Record }
      : initialValues;
    setReEntryVisaApplication(cleanValues.reEntryVisaApplication);
    setImmigrationApprovalDate(cleanValues.immigrationApprovalDate);
    setReEntryVisaExpiry(cleanValues.reEntryVisaExpiry);
    setAirTicketStatus(cleanValues.airTicketStatus);
    setRemarks(cleanValues.remarks);
    setDependentName(cleanValues.dependentName);
    setDependentPassportNumber(cleanValues.dependentPassportNumber);
    setDependentPassportExpiry(cleanValues.dependentPassportExpiry);
    setRelation(cleanValues.relation);
    setLabourDepositPaidBy(cleanValues.labourDepositPaidBy);
    setLabourDepositReceiptNumber(cleanValues.labourDepositReceiptNumber);
    setLabourDepositAmount(cleanValues.labourDepositAmount);
    setLabourUploadDoc(cleanValues.labourUploadDoc ?? []);
    setCurrentLabourUploadDocValue("");
    setErrors({});
  };
  const [labourDetails2Record, setLabourDetails2Record] = React.useState(
    labourDetails2ModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getLabourDetails2.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getLabourDetails2
        : labourDetails2ModelProp;
      setLabourDetails2Record(record);
    };
    queryData();
  }, [idProp, labourDetails2ModelProp]);
  React.useEffect(resetStateValues, [labourDetails2Record]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateLabourDetails2.replaceAll("__typename", ""),
            variables: {
              input: {
                id: labourDetails2Record.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "LabourDetails2UpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || labourDetails2ModelProp)}
