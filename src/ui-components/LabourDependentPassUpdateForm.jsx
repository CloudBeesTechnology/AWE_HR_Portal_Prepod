...
export default function LabourDependentPassUpdateForm(props) {
  const {
    id: idProp,
    labourDependentPass: labourDependentPassModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = labourDependentPassRecord
      ? { ...initialValues, ...labourDependentPassRecord }
      : initialValues;
    ...
    setDependentPassportExpiy(cleanValues.dependentPassportExpiy);
  ...
  const [labourDependentPassRecord, setLabourDependentPassRecord] =
    React.useState(labourDependentPassModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(LabourDependentPass, idProp)
        : labourDependentPassModelProp;
      setLabourDependentPassRecord(record);
    };
    queryData();
  }, [idProp, labourDependentPassModelProp]);
  React.useEffect(resetStateValues, [labourDependentPassRecord]);
  const validations = {
    empID: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            LabourDependentPass.copyOf(labourDependentPassRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "LabourDependentPassUpdateForm")}
      {...rest}
    >
      <TextField
        label="Emp id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || labourDependentPassModelProp)}
