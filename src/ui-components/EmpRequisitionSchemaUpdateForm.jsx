...
import { getEmpRequisitionSchema } from "../graphql/queries";
import { updateEmpRequisitionSchema } from "../graphql/mutations";
const client = generateClient();
export default function EmpRequisitionSchemaUpdateForm(props) {
  const {
    id: idProp,
    empRequisitionSchema: empRequisitionSchemaModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = empRequisitionSchemaRecord
      ? { ...initialValues, ...empRequisitionSchemaRecord }
      : initialValues;
  ...
  const [empRequisitionSchemaRecord, setEmpRequisitionSchemaRecord] =
    React.useState(empRequisitionSchemaModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getEmpRequisitionSchema.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getEmpRequisitionSchema
        : empRequisitionSchemaModelProp;
      setEmpRequisitionSchemaRecord(record);
    };
    queryData();
  }, [idProp, empRequisitionSchemaModelProp]);
  React.useEffect(resetStateValues, [empRequisitionSchemaRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateEmpRequisitionSchema.replaceAll("__typename", ""),
            variables: {
              input: {
                id: empRequisitionSchemaRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "EmpRequisitionSchemaUpdateForm")}
      {...rest}
    >
      <TextField
        label="Department"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || empRequisitionSchemaModelProp)}
