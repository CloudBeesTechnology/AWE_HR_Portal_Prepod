...
import { getWorkInfoSchema } from "../graphql/queries";
import { updateWorkInfoSchema } from "../graphql/mutations";
const client = generateClient();
export default function WorkInfoSchemaUpdateForm(props) {
  const {
    id: idProp,
    workInfoSchema: workInfoSchemaModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = workInfoSchemaRecord
      ? { ...initialValues, ...workInfoSchemaRecord }
      : initialValues;
    ...
    setReasonOfTermination(cleanValues.reasonOfTermination);
    setDestinationOfEntitlement(cleanValues.destinationOfEntitlement);
    setLeavePassageEntitlement(cleanValues.leavePassageEntitlement);
    setDateOfEntitlement(cleanValues.dateOfEntitlement);
    setDurationPeriodEntitlement(cleanValues.durationPeriodEntitlement);
    setAnnualLeaveEntitlement(cleanValues.annualLeaveEntitlement);
    setEffectiveDateRevision(cleanValues.effectiveDateRevision);
  ...
  const [workInfoSchemaRecord, setWorkInfoSchemaRecord] = React.useState(
    workInfoSchemaModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getWorkInfoSchema.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getWorkInfoSchema
        : workInfoSchemaModelProp;
      setWorkInfoSchemaRecord(record);
    };
    queryData();
  }, [idProp, workInfoSchemaModelProp]);
  React.useEffect(resetStateValues, [workInfoSchemaRecord]);
  const validations = {
    empID: [{ type: "Required" }],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          sickLeaveEntitlement,
          jobCategory,
          otherJobCategory: otherJobCategory ?? null,
          upgradeDate,
          normalWorkingWeek,
          normalWorkingMonth,
          effectiveDateOfSickLeave,
          ...
          ContractEffectDate,
          contractOfEmployee,
          remarksWorkInfo: remarksWorkInfo ?? null,
        };
        ...
        try {
          ...
          await client.graphql({
            query: updateWorkInfoSchema.replaceAll("__typename", ""),
            variables: {
              input: {
                id: workInfoSchemaRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "WorkInfoSchemaUpdateForm")}
      {...rest}
    >
      <TextField
        label="Emp id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || workInfoSchemaModelProp)}
