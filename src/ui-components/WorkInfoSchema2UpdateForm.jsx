...
import { getWorkInfoSchema2 } from "../graphql/queries";
import { updateWorkInfoSchema2 } from "../graphql/mutations";
const client = generateClient();
export default function WorkInfoSchema2UpdateForm(props) {
  const {
    id: idProp,
    workInfoSchema2: workInfoSchema2ModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = workInfoSchema2Record
      ? { ...initialValues, ...workInfoSchema2Record }
      : initialValues;
  ...
  const [workInfoSchema2Record, setWorkInfoSchema2Record] = React.useState(
    workInfoSchema2ModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getWorkInfoSchema2.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getWorkInfoSchema2
        : workInfoSchema2ModelProp;
      setWorkInfoSchema2Record(record);
    };
    queryData();
  }, [idProp, workInfoSchema2ModelProp]);
  React.useEffect(resetStateValues, [workInfoSchema2Record]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateWorkInfoSchema2.replaceAll("__typename", ""),
            variables: {
              input: {
                id: workInfoSchema2Record.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "WorkInfoSchema2UpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || workInfoSchema2ModelProp)}
