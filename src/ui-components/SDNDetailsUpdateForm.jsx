...
import { getSDNDetails } from "../graphql/queries";
...
export default function SDNDetailsUpdateForm(props) {
  const {
    id: idProp,
    sDNDetails: sDNDetailsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = sDNDetailsRecord
      ? { ...initialValues, ...sDNDetailsRecord }
      : initialValues;
  ...
  const [sDNDetailsRecord, setSDNDetailsRecord] =
    React.useState(sDNDetailsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getSDNDetails.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getSDNDetails
        : sDNDetailsModelProp;
      setSDNDetailsRecord(record);
    };
    queryData();
  }, [idProp, sDNDetailsModelProp]);
  React.useEffect(resetStateValues, [sDNDetailsRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateSDNDetails.replaceAll("__typename", ""),
            variables: {
              input: {
                id: sDNDetailsRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "SDNDetailsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Emp id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || sDNDetailsModelProp)}
