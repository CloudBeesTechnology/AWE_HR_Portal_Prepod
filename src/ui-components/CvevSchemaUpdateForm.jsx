...
import { getCvevSchema } from "../graphql/queries";
import { updateCvevSchema } from "../graphql/mutations";
const client = generateClient();
export default function CvevSchemaUpdateForm(props) {
  const {
    id: idProp,
    cvevSchema: cvevSchemaModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = cvevSchemaRecord
      ? { ...initialValues, ...cvevSchemaRecord }
      : initialValues;
    setCvecApproveDate(cleanValues.cvecApproveDate);
    setCvecFile(cleanValues.cvecFile);
    setErrors({});
  };
  const [cvevSchemaRecord, setCvevSchemaRecord] =
    React.useState(cvevSchemaModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getCvevSchema.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getCvevSchema
        : cvevSchemaModelProp;
      setCvevSchemaRecord(record);
    };
    queryData();
  }, [idProp, cvevSchemaModelProp]);
  React.useEffect(resetStateValues, [cvevSchemaRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateCvevSchema.replaceAll("__typename", ""),
            variables: {
              input: {
                id: cvevSchemaRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "CvevSchemaUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || cvevSchemaModelProp)}
