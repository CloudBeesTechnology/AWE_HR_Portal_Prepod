    ...
    useEffect(() => {
      const fetchData = async () => {
        ...
        console.log("seperatedGroupedData : ", seperatedGroupedData);
          ...
          // Calculate the number of days between the startDate and endDate (inclusive)
          const dayCount = (endDate - startDate) / (1000 * 60 * 60 * 24) + 1;
          
          // Flatten holiday dates and reformat them
          ...
          const formattedHolidayDates = holidayDates.map((dateStr) => {
            ...
            return `${monthNumber}/${dayNumber}/${year}`; // Format as "MM/DD/YYYY"
          });
          
          // Generate all dates between the startDate and endDate
          ...
          // Filter out dates that match formattedHolidayDates
          const holidayMatches = dateList.filter((date) =>
            formattedHolidayDates.includes(date)
          );
          
          // Output only the holiday-matching dates
          console.log("Holiday Dates Matching:", holidayMatches);
        ...
        const addLeaveTypeCount = seperatedGroupedData?.map((val) => {
          ...
          const startDate = new Date("1-10-2024");
          const endDate = new Date("31-10-2024");
            ...
            if (dayOfWeek === "Saturday") {
              ...
              if (!checkEntry) {
                ...
                if (workingHrs < entry?.normalWorkHrs) {
                  const absence = (entry?.normalWorkHrs - workingHrs).toFixed(
                    1
                  );
        ...
        addLeaveTypeCount.forEach((data) => {
          const { newDate, workingHrs } = data;
        // console.log(workingHrs)
          // Check if newDate matches any holiday
          if (holidayMatches.includes(newDate)) {
            const holidayDate = new Date(newDate);
            const day = holidayDate.getDate(); // Extract day from date
            if (workingHrs[day]) {
              workingHrs[day] = "PH"; // Assign "PH" for Public Holiday
            }
          }
        });
