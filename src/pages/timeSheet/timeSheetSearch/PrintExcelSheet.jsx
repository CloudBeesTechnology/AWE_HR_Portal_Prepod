  ...
  const buildRowsHtml = () => {
    ...
    return rows
      .map((emp, idx) => {
        ...
        // only add Grand Total after last employee
        const grandTotalRow =
          idx === rows.length - 1
            ? `
        <tr class="totals grand-total" style="font-weight:bold;">
          <td class="total-label">Grand Total</td>
          <td class="" ></td>
          ${Array.from({ length: dayCounts })
            .map(() => `<td style="border-left: none; border-right: none;"></td>`)
            .join("")}
          <td>${grandTotals.totalHours.toFixed(2)}</td>
          <td>${grandTotals.normalDays.toFixed(2)}</td>
          <td>${grandTotals.PH}</td>
          <td>${grandTotals.PHD}</td>
          <td>${grandTotals.ALCL}</td>
          <td>${grandTotals.SL}</td>
          <td>${grandTotals.OFF}</td>
          <td>${grandTotals.ABS.toFixed(2)}</td>
          <td>${grandTotals.UAL}</td>
          <td>${grandTotals.OT.toFixed(2)}</td>
          <td></td>
          <td></td>
        </tr>
      `
