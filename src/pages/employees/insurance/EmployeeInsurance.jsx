...
export const EmployeeInsurance = () => {
  ...
  useEffect(() => {
    if (empStatusTypeValue) {
      ...
      if (filteredWorkMen.length > 0) {
        setValue("workmenCompNo", filteredWorkMen[0].workmenCompNo);
      } else {
        setValue("workmenCompNo", ""); // Clear if no matching records
      }
    }
  }, [empStatusTypeValue, setValue, workMenDetails]);
      ...
      <div className="form-group">
        ...
        >
          {workMenDetails
            .filter((item) => {
              const currentDate = new Date();
              const expiryDate = new Date(item.workmenCompExp); // Assuming workmenCompExp is the expiry date
              return (
                item.empStatusType === empStatusTypeValue && expiryDate > currentDate
              );
            })
...
//   // State to track selected empStatusType
//   const [empStatusTypeValue, setEmpStatusTypeValue] = useState("");

//   // UseEffect to set the workmenCompNo when empStatusType is selected
//   useEffect(() => {
//     if (empStatusTypeValue) {
//       // Filter, sort, and exclude expired policies
//       const filteredWorkMen = workMenDetails
//         .filter((item) => {
//           const currentDate = new Date();
//           const expiryDate = new Date(item.workmenCompExp); // Assuming workmenCompExp is the expiry date
//           return (
//             item.empStatusType === empStatusTypeValue && expiryDate > currentDate
//           ); // Only include non-expired policies for the selected status type
//         })
//         .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)); // Sort by createdAt (latest first)

//       if (filteredWorkMen.length > 0) {
//         // Set the workmenCompNo to the most recent item
//         setValue("workmenCompNo", filteredWorkMen[0].workmenCompNo);
//       } else {
//         setValue("workmenCompNo", ""); // Clear if no matching records
//       }
...
//   // Default options in Workmen Compensation Insurance dropdown
//   const defaultOptions = ["OffShore", "OnShore", "General"];

//   // Handle form submission
//   const onSubmit = (data) => {
//     console.log("Form Submitted:", data);
//     // You can perform any action here like API call or state updates
//   };
...
//           ))}
//         </select>
//         {errors.empStatusType && (
//           <p className="text-red-500">{errors.empStatusType.message}</p>
//         )}
//       </div>
...
//             ))}
//         </select>
//         {errors.workmenCompNo && (
//           <p className="text-red-500">{errors.workmenCompNo.message}</p>
//         )}
//       </div>
