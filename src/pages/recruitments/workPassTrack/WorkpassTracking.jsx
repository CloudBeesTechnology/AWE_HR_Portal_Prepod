...
export const WorkpassTracking = () => {
  ...
  const handleFilterChange = (event) => {
    ...
    const newFilteredData = data.filter(item => item.workpasstracking === selectedValue);
  ...
  const columns = ['TempId', 'Name', 'Nationality', 'Position', 'Contract', 'Type', 'Email', 'Contact'];
    ...
    <section className="screen-size min-h-screen mb-4 "> 
      <div className="relative">
        ...
        <button
          ...
          className={`absolute top-0 right-10 px-6 py-2 font-semibold rounded-lg flex items-center ${selectedFilters.length ? 'bg-[#faf362]' : 'bg-[#8d8f9036]'}`}
        >
        ...
        {isFilterBoxOpen && (
          ...
          >
            {['SAWP', 'DOE', 'NLMS', 'Bank Guarantee', 'JITPA', 'Labour Deposit','Immigration', 'Air Ticket', 'NonLocal Mobilization'].map((workpasstracking) => (
              <label
                key={workpasstracking} 
                className={`flex items-center font-semibold space-x-2 hover:font-bold hover:text-[#c7bd03] p-2 rounded-md ${selectedFilters === workpasstracking ? 'text-[#faf362]' : 'text-lite_grey'}`}
              >
                <input 
                  type="radio" 
                  className="w-5 h-6" 
        ...
        {!loading && !error ? (
          ...
          <div>{loading ? "Loading..." : "Error fetching data. Check your Internet connection."}</div>
        )}
