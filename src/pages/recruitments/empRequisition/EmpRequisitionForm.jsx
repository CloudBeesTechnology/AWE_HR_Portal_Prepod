...
import { fetchGMAndHRDetails } from "../../../services/readMethod/fetchGMAndHRDetails";
...
export const EmpRequisitionForm = ({ isVisible, onClose }) => {
  ...
  const [gmAndHREmails, setGMAndHREmails] = useState({
    generalManagerEmail: null,
    humanResourceManagerEmail: null,
  });
  ...
  useEffect(() => {
    ...
    // Fetch GM and HR emails
    const fetchEmails = async () => {
      try {
        const emails = await fetchGMAndHRDetails();
        setGMAndHREmails(emails);
        console.log("Fetched GM and HR Emails:", emails);
      } catch (error) {
        console.error("Error fetching GM and HR emails:", error);
      }
    };
    fetchEmails();
  }, []);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const { generalManagerEmail, humanResourceManagerEmail } = gmAndHREmails;
  
      if (userType === "Manager") {
        if (generalManagerEmail) {
          console.log("Sending email to GM at:", generalManagerEmail);
          await sendEmail(
            `Employee Requisition Request`,
            `Dear GM, 
            Your Manager ${personalInfo.name || "N/A"} has submitted the Employee Requisition Form.
        ...
        if (humanResourceManagerEmail) {
          console.log("Sending email to HR at:", humanResourceManagerEmail);
          await sendEmail(
            `Employee Requisition Request`,
            `Dear HR, 
            Your Manager ${personalInfo.name || "N/A"} has submitted the Employee Requisition Form.
  ...
  return (
    <div className="bg-white w-full max-w-[900px] rounded-lg relative">
      <div className="text-center p-2">
        <img src={AweLogo} alt="Logo" className="max-w-[250px] w-full mx-28 mb-3"/>
