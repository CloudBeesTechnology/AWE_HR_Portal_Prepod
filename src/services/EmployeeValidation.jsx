...
export const LabourImmigrationSchema = Yup.object().shape({
  empID: Yup.string().required("Employee ID Number is mandatory"),
  overMD: Yup.string().required(
    "Overseas Medical Fitness Issued Date is mandatory"
  ),
  overME: Yup.string().required("Overseas Medical Fitness Expiry is mandatory"),
  bruhimsRD: Yup.string().required("BruHims Registration Date is mandatory"),
  bruhimsRNo: Yup.string().required("BruHims Registration Number is mandatory"),
  bruneiMAD: Yup.string().required(
    "Brunei Medical Appointment Date is mandatory"
  ),
  bruneiME: Yup.string().required("Brunei Medical Fitness Expiry is mandatory"),
  uploadFitness: Yup.mixed().test(
    "fileType",
    "Upload must be a PDF file",
    (value) => (value ? value.type === "application/pdf" : true)
  ),
  uploadRegis: Yup.mixed().test(
    "fileType",
    "Upload must be a PDF file",
    (value) => (value ? value.type === "application/pdf" : true)
  ),
  uploadBwn: Yup.mixed().test(
    "fileType",
    "Upload must be a PDF file",
    (value) => (value ? value.type === "application/pdf" : true)
  ),
  dependPass: Yup.array().of(
    Yup.object().shape({
      ...
      uploadDp: Yup.mixed().test(
        "fileType",
        "Upload must be a PDF file",
        (value) => (value ? value.type === "application/pdf" : true)
      ),
      uploadDr: Yup.mixed().test(
        "fileType",
        "Upload must be a PDF file",
        (value) => (value ? value.type === "application/pdf" : true)
      ),
  ...
  // workPermitType: Yup.string().required("Work Permit Type is mandatory"),
  // arrivalStampingExpiry: Yup.string().required("Arrival Stamping Expiry is mandatory"),
  // employmentPassEndorsement: Yup.string().required("Employment Pass Endorsement is mandatory"),
  // immigrationDeptDate: Yup.string().required("Immigration Dept Date is mandatory"),
  // employmentPassExpiry: Yup.string().required("Employment Pass Expiry is mandatory"),
  // employmentPassStatus: Yup.string().required("Employment Pass Status is mandatory"),
  // passportLocation: Yup.string().required("Original Passport Location is mandatory"),
  // reEntryVisaApplication: Yup.string().required("Re-entry Visa Application is mandatory"),
  // immigrationApprovalDate: Yup.string().required("Date Approved by Immigration Dept is mandatory"),
  // reEntryVisaExpiry: Yup.string().required("Re-entry Visa Expiry is mandatory"),
  // airTicketStatus: Yup.string().required("Air ticket status is mandatory"),
  // remarks: Yup.string().required("Remarks for Labor/Immigration are mandatory"),
});
...
export const WorkInfoSchema = Yup.object().shape({
  ...
  contractEnd: Yup.string()
    ...
    .test(
      "is-after-doj",
      "Contract End date must be after Date of Joining",
      function (value) {
        const { doj } = this.parent;
        if (!doj || !value) return true;
        const dojDate = new Date(doj);
        const contractEndDate = new Date(value);
        return contractEndDate > dojDate;
      }
    ),
  ...
  workInfoUploads: Yup.string().notRequired(),
...
export const employeeInfoSchema = Yup.object().shape({
  ...
  profilePhoto: Yup.mixed()
    ...
    .test(
      "fileType",
      "Profile photo must be a JPG, PNG, or a valid URL",
      (value) =>
        value
          ? typeof value === "string"
  ...
  bwnIcExpiry: Yup.string()
    .notRequired() // This allows the field to be optional
    .test("is-future-date", "Expiry date must be in the future", (value) => {
      if (!value) return true; // If no value, allow it to pass (since it's not required)
  ...
  ppIssued: Yup.string()
    .notRequired() // This allows the field to be optional
    .test("is-past-date", "Expiry date must be in the past", (value) => {
      if (!value) return true; // If no value, allow it to pass (since it's not required)
      const expiryDate = new Date(value);
      const currentDate = new Date();
      return expiryDate < currentDate;
  ...
  sapNo: Yup.string().required("SAP Number is mandatory"),
  ...
  inducBriefUp: Yup.mixed()
    .test(
      "fileType",
      "Only PDF, image files, or URLs are allowed",
      (value) =>
        !value ||
        ["application/pdf", "image/jpeg", "image/png"].includes(value?.type) ||
        (typeof value === "string" && isValidUrl(value))
    )
    .required("Induction Briefing Upload is mandatory"),
